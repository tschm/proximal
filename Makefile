...
# The caveat in the comment above -- "a *file* sharing a task's name would shadow it -- none
# do" -- is not true in this repo, so the two names that collide are declared here. `book/`
# is a directory and `book` is a task, so make's up-to-date check on the directory wins and
# prints "`book' is up to date" instead of building the site. `license` collides with
# `LICENSE` only on a case-insensitive filesystem, so it shadows on macOS and not on the
# Linux runners -- which is exactly the kind of divergence worth pinning down explicitly.
# Both need an *explicit* recipe, not just `.PHONY`: make skips the implicit- and
# pattern-rule search for phony targets, so declaring these phony and leaving them to `%:`
# trades "`book' is up to date" for "Nothing to be done for `book'" -- still not a build.
# `.PHONY` stops the filesystem check; the recipe below is what actually reaches the task.
.PHONY: book license
book license: $(UVX)
	@$(UVX) $(RHIZA_TASK) $@
