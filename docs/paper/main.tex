...
\begin{algorithm}[t]
\caption{Simplex projection \citep{duchi2008}}
\label{alg:proj}
\begin{algorithmic}[1]
\Require $v \in \R^n$, radius $s > 0$
\State $\mu \gets \operatorname{sort}(v)$ in decreasing order
\State $\theta_j \gets \bigl(\sum_{i \le j} \mu_i - s\bigr) / j$
  \Comment{running mean of the top $j$, offset by $s$}
\State $\rho \gets \max \{ j : \mu_j > \theta_j \}$
\State \Return $x_i = \max(v_i - \theta_\rho, 0)$ for each $i$
\end{algorithmic}
\end{algorithm}
